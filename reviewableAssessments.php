...
abstract class reviewableAssessments extends frontControllerApplication
{
	...
	private function listSubmissions ($statuses = false, $heading = false, $reviewingMode = false)
	{
		...
		foreach ($submissions as $id => $submission) {
			...
			$statusDescription = ucfirst ($submission['status']);
			$table[$id]['title'] = "\n<a href=\"{$this->baseUrl}/submissions/{$id}/\"><img src=\"/images/icons/{$icon}.png\" alt=\"\" title=\"{$statusDescription}\" class=\"icon\" /> " . htmlspecialchars ($submission['description']) . '</a>';
			if (method_exists ($this, 'choiceForm')) {
				$colour = (isSet ($this->formColours[$submission['form']]) ? $this->formColours[$submission['form']] : false);
